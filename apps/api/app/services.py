...
CREDIT_GUIDANCE = {
    "IPc1": {"action": "Revise the integrative-process narrative to show the option considered, metric tested, decision owner and resulting design change.", "checks": ["workshop date and attendees", "energy/water/carbon option matrix", "decision log linked to drawings"], "steps": ["Add a one-page workshop record with attendees, agenda, options and decisions.", "Add an option matrix with energy, water, carbon and cost metrics.", "Cross-reference each selected decision to the affected drawing/specification sheet."]},
    "IPp1": {"action": "Add the integrative-process prerequisite record before claiming prerequisite completion.", "checks": ["owner goals", "team workshop", "energy and water analysis", "documented decisions"], "steps": ["Create an owner/design-team workshop agenda and attendance record.", "Attach the initial energy and water analysis with assumptions.", "Add an action log showing owner, due date and design response for every unresolved item."]},
    "LTp1": {"action": "Correct the location/transportation package so the site plan, parking count and alternative-transport assumptions reconcile.", "checks": ["site location and transit", "parking ratio/count", "bike/EV facilities", "pedestrian access"], "steps": ["Annotate the site plan with transit stops, walking routes, bike storage and EV-ready spaces.", "Reconcile parking counts between site plan, code summary and LEED form.", "Add a signed transportation calculation with project area, occupancy and assumptions."]},
    "SSc1": {"action": "Complete a documented site assessment and show how the findings changed the concept.", "checks": ["topography", "hydrology", "soils", "vegetation/habitat", "land-use context"], "steps": ["Add a site constraints plan with north arrow, limits of disturbance and survey sources.", "Provide soil, hydrology, vegetation and habitat findings with dates and authors.", "Mark each finding as avoided, protected or mitigated on the landscape/civil drawings."]},
    "SSc2": {"action": "Revise the biodiversity package to quantify habitat protection/restoration and native planting outcomes.", "checks": ["existing habitat baseline", "native/adapted planting schedule", "invasive species", "tree canopy/habitat area", "light pollution"], "steps": ["Add a habitat baseline plan with existing vegetation, water features and protected areas.", "Add a planting schedule identifying native/adapted status, quantities and mature coverage.", "Calculate habitat/landscape areas and show them consistently on the site plan and landscape schedule.", "Add invasive-species and exterior-lighting controls with drawing references."]},
    "WEp1": {"action": "Provide the water-performance prerequisite calculations and fixture/irrigation evidence.", "checks": ["baseline fixture flow", "proposed flow", "occupancy/use assumptions", "irrigation demand", "metering boundary"], "steps": ["Add a fixture schedule with manufacturer, flow/flush rate and installed location.", "Show baseline-versus-proposed water calculations by fixture type.", "Reconcile plumbing drawings, irrigation plan, water balance and meter points."]},
    "WEc1": {"action": "Quantify indoor and outdoor water reduction rather than citing fixtures generically.", "checks": ["fixture count", "flow/flush rates", "irrigation zones", "monthly water balance", "non-potable sources"], "steps": ["Add fixture quantities and rates to the water calculation workbook.", "Add irrigation zones, plant factors, efficiency and monthly demand.", "Identify every non-potable source, storage volume, end use and control point."]},
    "EAp1": {"action": "Add the fundamental energy-performance baseline and signed commissioning scope.", "checks": ["ASHRAE baseline", "proposed model", "energy sources", "metering", "commissioning scope"], "steps": ["Submit the baseline/proposed model files and model input/output summary.", "Add utility meter schedule and energy-source boundary diagram.", "Name the CxA and attach commissioning plan, systems list and issue log."]},
    "EAc1": {"action": "Add a traceable baseline-versus-proposed energy model and identify the design measures that create the savings.", "checks": ["baseline/proposed model", "EUI and savings", "envelope/HVAC schedules", "renewables", "model calibration"], "steps": ["Add model version, software, weather file, utility rates and floor-area assumptions.", "Provide baseline/proposed EUI and energy-use-by-end-use tables.", "Mark envelope, lighting, HVAC, controls and renewable inputs on drawings/specifications.", "Reconcile model areas and system names with plans and schedules; include signed modeller QA."]},
    "EAc2": {"action": "Show whole-building and end-use meter locations, points list and data access responsibility.", "checks": ["utility meter", "end-use submeters", "electrical one-line", "trend interval", "data owner"], "steps": ["Annotate meter IDs and locations on the electrical/mechanical one-lines.", "Add a points list with end use, interval, units and data owner.", "Confirm the narrative, meter schedule and commissioning test record agree."]},
    "EAc3": {"action": "Add water-meter boundary and end-use coverage evidence.", "checks": ["whole-building water meter", "irrigation/process meters", "plumbing riser", "data access"], "steps": ["Mark all meters and submeters on plumbing drawings.", "Provide a meter schedule with served area/end use and accuracy.", "Add commissioning or trend evidence showing the meters are operational."]},
    "EAc4": {"action": "Complete enhanced commissioning documentation and close every issue before submission.", "checks": ["CxA independence", "OPR/BOD", "systems testing", "issues log", "final report"], "steps": ["Attach OPR, BOD and commissioning plan with responsible parties.", "Add functional test scripts/results for HVAC, lighting, controls and water systems.", "Close the issues log and attach the final commissioning report with dates/signatures."]},
    "EAc5": {"action": "Document renewable-energy/storage capacity, annual production and the electrical interconnection boundary.", "checks": ["PV/renewable capacity", "annual generation", "storage capacity", "one-line", "ownership"], "steps": ["Add equipment schedule, capacity, orientation and annual production calculation.", "Annotate renewable and storage equipment on roof/electrical drawings.", "Reconcile production, load assumptions, utility interconnection and ownership narrative."]},
    "MRp1": {"action": "Create the materials evidence register before claiming the materials prerequisite.", "checks": ["materials inventory", "quantity basis", "product locations", "manufacturer evidence", "scope boundary"], "steps": ["Create a product register with manufacturer, model, quantity, location and CSI/MasterFormat.", "Define the project material scope and exclude/include items consistently.", "Link every register row to an EPD/HPD/source document and specification section."]},
    "MRc1": {"action": "Map each counted product to a valid manufacturer EPD and quantity calculation.", "checks": ["EPD program/operator", "validity date", "product/model", "quantity", "cost/area basis"], "steps": ["Add EPD number, program operator, issue/expiry date and product match.", "Add quantity and cost/area calculation for every counted product.", "Flag generic or expired EPDs and replace them before submission."]},
    "MRc2": {"action": "Add responsible-sourcing evidence for each counted material and reconcile procurement quantities.", "checks": ["manufacturer declaration", "recycled content", "FSC/chain of custody", "regional source", "quantity"], "steps": ["Add a material-by-material sourcing register with certification number and validity.", "Attach chain-of-custody or manufacturer declarations to each product row.", "Reconcile procurement log, specification and installed quantities."]},
    "MRc3": {"action": "Complete construction-waste tracking with stream weights, destinations and diversion calculation.", "checks": ["waste streams", "weights/receipts", "destination", "diversion rate", "contamination"], "steps": ["Add monthly waste log by stream, weight/volume and hauler.", "Attach receipts and destination/recycling facility evidence.", "Recalculate diversion and reconcile totals with contractor reports and project dates."]},
    "EQp1": {"action": "Provide ventilation calculations, outdoor-air schedules and commissioning records.", "checks": ["ASHRAE 62.1 basis", "outdoor-air rates", "zone schedule", "MERV/filtration", "CO2 monitoring", "air-balancing"], "steps": ["Add an outdoor-air schedule by zone with occupancy, area, required and provided flow.", "Annotate outdoor-air intakes, sensors, filters and airflow direction on MEP plans.", "Attach air-balance/commissioning results and resolve any zone shortfalls."]},
    "EQc1": {"action": "Show enhanced IAQ strategies on the MEP drawings and verify their operation.", "checks": ["entryway system", "filtration", "flush-out/air testing", "isolation", "CO2 monitoring"], "steps": ["Mark entryway walk-off/grille locations and cross-contamination controls on plans.", "Add filter grade, pressure drop and replacement responsibility to the schedule.", "Attach flush-out or air-testing procedure/results with dates and responsible party."]},
    "EQc2": {"action": "Replace generic material lists with a low-emitting product schedule and VOC/emissions evidence.", "checks": ["product/model", "installed location", "VOC/emissions certificate", "wet-applied products", "composite wood"], "steps": ["Add manufacturer/model and installed location for every counted product.", "Attach current VOC/emissions certificates and identify test standard.", "Flag products without evidence and revise specifications before procurement."]},
    "EQc3": {"action": "Provide a reproducible daylight simulation with model scope, assumptions and annotated result plans.", "checks": ["model geometry", "weather file", "sDA/ASE or approved metric", "glazing/shading", "result plots"], "steps": ["Annotate simulation zones and excluded spaces on floor plans.", "State software/version, weather file, reflectance, glazing and shading assumptions.", "Attach result plots/tables and revise glazing/shading where targets are missed."]},
    "INc1": {"action": "Define the innovation claim, measurable outcome and verification owner instead of using a generic innovation narrative.", "checks": ["innovation intent", "measurable metric", "baseline", "implementation", "verification"], "steps": ["Write a one-page innovation narrative with baseline, target and success metric.", "Identify affected drawings/specifications and responsible owner.", "Add a monitoring/verification plan and evidence source for the claimed outcome."]},
    "RPc1": {"action": "Confirm applicable regional-priority credits for the project location and document the qualifying strategy.", "checks": ["project location", "official regional-priority lookup", "qualifying credit", "threshold/evidence"], "steps": ["Add the location used for the official lookup and date accessed.", "Identify the selected regional-priority credit and qualifying threshold.", "Cross-reference the qualifying calculation/drawing to the submission narrative."]},
    "CAc1": {"action": "Quantify operational and embodied carbon reductions with a common project boundary and baseline.", "checks": ["operational carbon", "LCA boundary", "embodied carbon", "EPDs", "reduction scenarios"], "steps": ["Define life-cycle modules, study period, functional unit and project boundary.", "Add baseline and proposed carbon tables with assumptions and data sources.", "Link EPDs, energy model outputs and material quantities to each reduction claim."]},
    "HWc1": {"action": "Convert health goals into measurable occupant criteria and design evidence.", "checks": ["thermal comfort", "acoustics", "daylight", "biophilia", "occupant process"], "steps": ["Add comfort criteria, target ranges and calculation method.", "Annotate daylight, views, acoustic zones and biophilic elements on plans.", "Document occupant engagement and post-occupancy/verification plan."]},
    "CRc1": {"action": "Map climate hazards to design adaptations and verify them on drawings and specifications.", "checks": ["hazard baseline", "risk rating", "adaptation measure", "design location", "residual risk"], "steps": ["Add hazard sources, exposure/sensitivity assumptions and risk rating.", "Mark flood, heat, wildfire, wind or outage adaptations on drawings.", "Add residual-risk owner, maintenance requirement and verification evidence."]},
}
...
class MemoryStore:
    ...
    def _submission_requirement_review(self, module: CreditModule, documents: list[dict]) -> tuple[list[dict], list[str]]:
        ...
        guidance = CREDIT_GUIDANCE.get(module.credit_id, {"action": RULES.get(module.module_type, ("", "Map evidence to the credit requirements.", ""))[1], "checks": [], "steps": []})
        review: list[dict] = []
        actions: list[str] = []
        for requirement in requirements:
            ...
            if status == "missing":
                action = f"{guidance['action']} Add {name}; accepted types: {', '.join(accepted) or 'project evidence'}. Include: {', '.join(fields) or 'scope, calculation, conclusion'}; responsible party: {requirement.get('responsible_discipline', 'leed_consultant')}."
                steps = list(guidance.get("steps", [])) + [f"Assign final review to {requirement.get('responsible_discipline', 'leed_consultant')} before submission."]
            elif status == "needs_official_source":
                action = f"{guidance['action']} For {name}, cite the official {module.leed_version} {module.rating_family}/{module.adaptation} clause and map each required field ({', '.join(fields)}) to a page, sheet or calculation cell."
                steps = list(guidance.get("steps", [])) + [f"Insert the official {module.leed_version} {module.rating_family}/{module.adaptation} credit/prerequisite reference in the narrative."]
            else:
                action = f"{guidance['action']} Revise {name} to add explicit clause citation and page/sheet references for: {', '.join(fields) or 'scope and conclusion'}."
                steps = list(guidance.get("steps", [])) + ["Check consistency with project boundary, area and related credits."]
            review.append({"requirement": name, "status": status, "required_phase": requirement.get("required_phase", "submission"), "accepted_file_types": accepted, "required_fields": fields, "specific_checks": guidance.get("checks", []), "validation_rules": requirement.get("validation_rules", []), "responsible_discipline": requirement.get("responsible_discipline", "leed_consultant"), "matched_files": matched_files, "modification_comment": action, "specific_modification_steps": steps})
